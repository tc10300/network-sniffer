...
class Detect: 
    ...
    def detail(self, packet):
        ...
        black_color = "#1C1C1C"
        text = tk.Text(win, bg=black_color, fg="white")
        text.pack(fill="both", expand=True)
        frame = tk.Frame(win)
        frame.pack(anchor=tk.CENTER, side=tk.BOTTOM, fill=tk.X)
        button = ttk.Button(frame, text='返回', command=win.destroy)
        button.pack(anchor=tk.CENTER, side=tk.TOP, fill=tk.X)
    ...
    def transportLayer(self,packet,text):
        def udp_segment():
            try:
                packet_bytes = bytes(packet[UDP]) # type: ignore
        ...
        def tcp_segment():
            try:
                packet_bytes = bytes(packet[TCP])# type: ignore
                src_port,des_port,seq,AcknowledgeNum,offset,flags,windows,checksum,urgent = struct.unpack("!HHIIBBHHH", packet_bytes[:20])
                length=(offset>>4)*4
                urg=(flags&32)>>5
                ack=(flags&16)>>4
                psh=(flags&8)>>3
                ...
                text.insert(tk.END, f"Acknowledgment Number: {AcknowledgeNum}\n")
    ...
    def analyze_ip_packet(self,packet,text):
        def analyze_ipv4_packet():
            try:
                packet_bytes=bytes(packet[IP])# type: ignore
                ...
                text.insert(tk.END, "No IPv4 layer found in the packet.\n")
                text.insert(tk.END,"----------------------------------\n")  
        def analyze_ipv6_packet():
            try:
                ipv6_bytes = bytes(packet[IPv6])# type: ignore
                version_tc_flow, payload_length, next_header, hop_limit,src_addr,dst_addr = struct.unpack('!IHBB16s16s', ipv6_bytes[:40])
                version = version_tc_flow >> 28
                tc = (version_tc_flow >> 20) & 0x000000FF
                flow = version_tc_flow & 0x000FFFFF
                src_addr_str = ':'.join(f'{byte:04x}' for byte in src_addr)
                dst_addr_str = ':'.join(f'{byte:04x}' for byte in dst_addr)
                ...
                text.insert(tk.END, f"source ip: {src_addr_str}\n")
                text.insert(tk.END, f"destination ip: {dst_addr_str}\n")
                text.insert(tk.END, "----------------------------------\n")
            except IndexError:
                text.insert(tk.END, "No IPv6 layer found in the packet.\n")
                text.insert(tk.END,"----------------------------------\n")  
        if "IPv6" in packet:
            analyze_ipv6_packet()
        else:
    ...
    def analyze_ether(self,packet,text):
        ether_bytes=bytes(packet[Ether])# type: ignore
        if len(ether_bytes) < 14:
            text.insert(tk.END, "Packet bytes are too short")
        dst_addr, src_addr,ether_type= struct.unpack('!6s6sH', ether_bytes[0:14])
...
class ctl:
    ...
    def card_choose(self):
        ...
        label=tk.Label(self.card_frame,text="请选择网卡", font=custom_font)
        label.pack(anchor=tk.CENTER,side=tk.TOP,pady=100)
        for iface in psutil.net_if_stats():
            button=ttk.Button(self.card_frame,text=iface,command=lambda iface=iface,card_frame=self.card_frame,button_frame=self.button_frame:self.create(iface))
            button.pack(anchor=tk.CENTER,side=tk.TOP,fill=tk.X)
            
    def create(self,iface):
        self.card_frame.pack_forget()
        myShow = ListShow(self.root,iface)
        Return=ttk.Button(self.button_frame, text="返回", command=lambda :(self.button_frame.pack_forget(),myShow.clear(),self.card_choose()))
        Return.pack(side=tk.LEFT,anchor=tk.CENTER)
        start_1 = ttk.Button(self.button_frame, text="批量抓包", command=lambda :myShow.Sniff_count())
        start_1.pack(side=tk.LEFT,anchor=tk.CENTER)
        start_2 = ttk.Button(self.button_frame, text="动态抓包", command=lambda :myShow.Sniff_all())
        start_2.pack(side=tk.LEFT,anchor=tk.CENTER)
        stop = ttk.Button(self.button_frame, text="停止抓包", comma=lambda :myShow.terminate())
        stop.pack(side=tk.LEFT,anchor=tk.CENTER)


if __name__ == "__main__":
    root = tk.Tk()
    root.title('Hack Sniffer')
